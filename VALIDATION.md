...
| **The whole file was never loaded — the finding underneath all four rows above.** Two stacked faults produced one symptom, which is why it took nine deployments to isolate | (1) `/var/ossec/etc/rules` is **not parsed at all** on this `siemng` image, though `ossec.conf` lists it; a file there is inert. (2) Rule 100100's `<field name="file">\.(exe\|scr\|…)$</field>` is PCRE, which OS_Regex cannot compile, so analysisd rejected the file with `ERROR (5107)` and **refused to start** — taking all four rules with it, not just 100100. Nothing ever asked analysisd whether it accepted the ruleset, so both stayed invisible from the first commit | rules moved to `/var/ossec/ruleset/rules/9999-puc2-2c.xml`, which is provably read; 100100 marked `type="pcre2"`. A `wazuh-analysisd -t` gate now parses the ruleset **before** the restart and pulls a rejected file back off disk, and a `pre_tasks` guard on the NG-SIEM play clears a poisoned ruleset left by a prior run. `wazuh-logtest` confirms analysisd traverses, matches and alerts on rule 100102 |
...
- **Inherited exposure — rotation required at the source.** The vendored
  ...
  Docker Hub PAT (`dckr_pat_…`) and a password hash for the `ubuntu` user.
  **None of the three remains in the tree.** The DFIR-IRIS key went with the dead
  task files that held it; the PAT and the hash are now read from `ansible-vault`
  and both degrade safely when unset (the Docker Hub login is skipped, and the
  password field is omitted rather than blanked).
  They had been kept verbatim on the argument that byte-identity is what makes
  the substrate reuse auditable. That argument expired with the component trim,
  which already edits `docker_server` and `ng-siem` — the divergence is documented
  in README instead. Scrubbing them here does not un-publish them: the PAT and
  the hash are public upstream and the IRIS key is in this repo's history, so all
  three still need rotating (`validation/SECURITY_ROTATION.md`).
