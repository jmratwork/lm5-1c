...
NEW_CONTENT = {
    ...
    1: (
        ...
        "## Which system to open, and where to start\n"
        "Four nodes matter in this exercise. You reach each one the same way "
        "(Topology -> `Generate console URL`); what changes is the login user and "
        "whether that node's image also offers a desktop:\n\n"
        "| Node | Address | Log in as | The GUI gives you | You go here to |\n"
        "|---|---|---|---|---|\n"
        "| **`victim`** (start here) | 10.0.16.100 | `ubuntu` | Console | Arm and "
        "fire the scenario, read the payload hash, verify isolation and eradication "
        "(Phases 1, 2, 4) |\n"
        "| `ng-siem` | 10.0.16.70 | `ubuntu` | Console | Read the NG-SIEM alerts and "
        "correlation and run the evaluation (Phases 2, 5) |\n"
        "| `docker-server` | 10.0.16.60 | `ubuntu` | Console | Run the CTI-SS / CICMS "
        "/ NG-SOAR helpers (`cti_lookup.sh`, `case_lookup.sh`, `share_intel.sh`), the "
        "playbook library and `CREDENTIALS.txt` (Phases 2-5) |\n"
        "| `kali` | 10.0.16.50 | `debian` | Console **and a graphical desktop** | The "
        "analyst workstation: use its browser to reach the four dashboards. "
        "10.0.16.50 is also the simulated C2 address - you never log in as an "
        "attacker, you only use its desktop |\n\n"
        "**Only `kali` ships a graphical desktop.** `victim`, `ng-siem` and "
        "`docker-server` run server images, so opening them from the GUI gives you a "
        "console, not a desktop. Every graded answer is reachable from a console, so "
        "you can complete the whole exercise without a desktop at all.\n\n"
        "**Start on `victim`.** Phase 1 begins by arming the scenario from a "
        "`victim` console, so open that node first.\n\n"
